...
internal class TITAN : Photon.MonoBehaviour
{
	...
	private void findNearestFacingHero2()
	{
		...
		float num2 = ((abnormalType != 0) ? 180f : 100f);
		foreach (HERO player in MultiplayerManager.getPlayers())
		{
			...
			if (num3 < num)
			{
				...
				if (Mathf.Abs(0f - Mathf.DeltaAngle((0f - Mathf.Atan2(vector.z, vector.x)) * 57.29578f, baseGameObjectTransform.rotation.eulerAngles.y - 90f)) < num2)
				{
		...
		foreach (TITAN_EREN eren in MultiplayerManager.getErens())
		{
			...
			if (num4 < num)
			{
				...
				if (Mathf.Abs(0f - Mathf.DeltaAngle((0f - Mathf.Atan2(vector2.z, vector2.x)) * 57.29578f, baseGameObjectTransform.rotation.eulerAngles.y - 90f)) < num2)
				{
		...
		myHero = gameObject;
		if (obj != myHero && IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && PhotonNetwork.isMasterClient)
		{
			if (myHero == null)
			{
		...
		if (!hasDie)
		{
			if (IN_GAME_MAIN_CAMERA.gametype != 0)
			{
				if (base.photonView.isMine)
				{
					...
					if (abnormalType != AbnormalType.TYPE_CRAWLER && state != TitanState.attack && state != TitanState.down && state != TitanState.hit && state != TitanState.recover && state != TitanState.eat && state != TitanState.hit_eye && !hasDie && myDistance < 100f && myHero != null)
					{
						...
						float value2 = Mathf.Atan2(neck.position.y + myLevel * 2f - myHero.transform.position.y, myDistance) * 57.29578f;
				...
				else
				{
					...
					if (asClientLookTarget && myHero != null)
					{
						...
						float value4 = Mathf.Atan2(neck.position.y + myLevel * 2f - myHero.transform.position.y, myDistance) * 57.29578f;
			...
			else
			{
				...
				if (abnormalType != AbnormalType.TYPE_CRAWLER && state != TitanState.attack && state != TitanState.down && state != TitanState.hit && state != TitanState.recover && state != TitanState.hit_eye && !hasDie && myDistance < 100f && myHero != null)
				{
					...
					float value6 = Mathf.Atan2(neck.position.y + myLevel * 2f - myHero.transform.position.y, myDistance) * 57.29578f;
	...
	private bool longRangeAttackCheck()
	{
		if (abnormalType == AbnormalType.TYPE_PUNK && myHero != null && myHero.rigidbody != null)
		{
			...
			{
				float sqrMagnitude = (myHero.transform.position + vector - base.transform.position).sqrMagnitude;
	...
	private bool longRangeAttackCheck2()
	{
		if (abnormalType == AbnormalType.TYPE_PUNK && myHero != null)
		{
			...
			float f = 0f - Mathf.DeltaAngle((0f - Mathf.Atan2(vector2.z, vector2.x)) * 57.29578f, baseGameObjectTransform.rotation.eulerAngles.y - 90f);
			if (rockInterval > 0f)
			{
				rockInterval -= Time.deltaTime;
			}
			...
			{
				float sqrMagnitude = (myHero.transform.position + vector - baseTransform.position).sqrMagnitude;
	...
	private void setLevel(float level, int AI, int skinColor)
	{
		...
		if (skinColor != 0)
		{
			mainMaterial.GetComponent<SkinnedMeshRenderer>().material.color = skinColor switch
			{
	...
	public void update2()
	{
		...
		if (hasDie)
		{
			...
			if (dieTime > 2f && !hasDieSteam)
			{
				...
				if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
				{
					...
					obj.transform.position = baseTransform.Find("Amarture/Core/Controller_Body/hip").position;
		...
		else if (state == TitanState.attack)
		{
			if (attackAnimation == "combo")
			{
				...
				{
					GameObject gameObject = checkIfHitHand(baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R/hand_R_001"));
					if (gameObject != null)
					{
						Vector3 position = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
						if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
				...
				{
					GameObject gameObject2 = checkIfHitHand(baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_L/upper_arm_L/forearm_L/hand_L/hand_L_001"));
					if (gameObject2 != null)
					{
						Vector3 position2 = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
						if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
						{
							gameObject2.GetComponent<HERO>().die((gameObject2.transform.position - position2) * 15f * myLevel, isBite: false);
						}
						else if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && base.photonView.isMine && !gameObject2.GetComponent<HERO>().HasDied())
						{
							...
							{
								(gameObject2.transform.position - position2) * 15f * myLevel,
								false,
								(!nonAI) ? (-1) : base.photonView.viewID,
								base.name,
								true
			...
			if (attackCheckTimeA != 0f && baseAnimation["attack_" + attackAnimation].normalizedTime >= attackCheckTimeA && baseAnimation["attack_" + attackAnimation].normalizedTime <= attackCheckTimeB)
			{
				if (leftHandAttack)
				{
					GameObject gameObject3 = checkIfHitHand(baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_L/upper_arm_L/forearm_L/hand_L/hand_L_001"));
					if (gameObject3 != null)
					{
						Vector3 position3 = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
						if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
						{
							gameObject3.GetComponent<HERO>().die((gameObject3.transform.position - position3) * 15f * myLevel, isBite: false);
						}
						else if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && base.photonView.isMine && !gameObject3.GetComponent<HERO>().HasDied())
						{
							...
							{
								(gameObject3.transform.position - position3) * 15f * myLevel,
								false,
								(!nonAI) ? (-1) : base.photonView.viewID,
								base.name,
								true
				...
				{
					GameObject gameObject4 = checkIfHitHand(baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R/hand_R_001"));
					if (gameObject4 != null)
					{
						Vector3 position4 = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
						if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
						{
							gameObject4.GetComponent<HERO>().die((gameObject4.transform.position - position4) * 15f * myLevel, isBite: false);
						}
						else if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && base.photonView.isMine && !gameObject4.GetComponent<HERO>().HasDied())
						{
							...
							{
								(gameObject4.transform.position - position4) * 15f * myLevel,
								false,
								(!nonAI) ? (-1) : base.photonView.viewID,
								base.name,
								true
			...
			if (!attacked && attackCheckTime != 0f && baseAnimation["attack_" + attackAnimation].normalizedTime >= attackCheckTime)
			{
				...
				GameObject gameObject5 = ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.MULTIPLAYER || !base.photonView.isMine) ? ((GameObject)UnityEngine.Object.Instantiate(Resources.Load("FX/" + fxName), fxPosition, fxRotation)) : PhotonNetwork.Instantiate("FX/" + fxName, fxPosition, fxRotation, 0));
				if (nonAI)
				{
					gameObject5.transform.localScale = baseTransform.localScale * 1.5f;
				...
				{
					gameObject5.transform.localScale = baseTransform.localScale;
				}
				if (gameObject5.GetComponent<EnemyfxIDcontainer>() != null)
				{
					gameObject5.GetComponent<EnemyfxIDcontainer>().titanName = base.name;
				}
				float b = 1f - Vector3.Distance(currentCamera.transform.position, gameObject5.transform.position) * 0.05f;
				b = Mathf.Min(1f, b);
			...
			if (attackAnimation == "throw")
			{
				...
				if (throwRock != null && baseAnimation["attack_" + attackAnimation].normalizedTime >= 0.62f)
				{
					...
					if (myHero != null)
					{
						...
						float num3 = myHero.transform.position.y + 2f * myLevel - throwRock.transform.position.y;
			...
			if (attackAnimation == "jumper_0" || attackAnimation == "crawler_jump_0")
			{
				if (!attacked)
				{
					if (baseAnimation["attack_" + attackAnimation].normalizedTime >= 0.68f)
					{
						...
						{
							float num4 = 120f;
							Vector3 velocity = baseTransform.forward * speed + Vector3.up * num4;
							if (nonAI && abnormalType == AbnormalType.TYPE_CRAWLER)
							{
								num4 = 100f;
								float a = speed * 2.5f;
								a = Mathf.Min(a, 100f);
								velocity = baseTransform.forward * a + Vector3.up * num4;
							}
						...
						else
						{
							...
							Vector3 vector3 = myHero.transform.position + myHero.rigidbody.velocity * num10 + Vector3.up * 0.5f * num5 * num10 * num10;
							float y4 = vector3.y;
							float num11;
							if (num9 < 0f || y4 - y3 < 0f)
							{
								...
								Vector3 velocity2 = baseTransform.forward * a2 + Vector3.up * num11;
								baseRigidBody.velocity = velocity2;
								return;
							}
							float num12 = y4 - y3;
							float num13 = Mathf.Sqrt(2f * num12 / gravity);
							num11 = gravity * num13;
							num11 = Mathf.Max(30f, num11);
							Vector3 vector4 = (vector3 - baseTransform.position) / num10;
							...
							baseRigidBody.AddForce(force, ForceMode.VelocityChange);
							baseRigidBody.AddForce(Vector3.up * num11, ForceMode.VelocityChange);
							float num14 = Vector2.Angle(new Vector2(baseTransform.position.x, baseTransform.position.z), new Vector2(myHero.transform.position.x, myHero.transform.position.z));
							num14 = Mathf.Atan2(myHero.transform.position.x - baseTransform.position.x, myHero.transform.position.z - baseTransform.position.z) * 57.29578f;
							baseGameObjectTransform.rotation = Quaternion.Euler(0f, num14, 0f);
						}
					}
					else
					{
				...
				GameObject gameObject6 = checkIfHitHead(head, 3f);
				if (gameObject6 != null)
				{
					Vector3 position5 = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
					if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
					{
						gameObject6.GetComponent<HERO>().die((gameObject6.transform.position - position5) * 15f * myLevel, isBite: false);
					}
					else if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && base.photonView.isMine && !gameObject6.GetComponent<HERO>().HasDied())
					{
						...
						{
							(gameObject6.transform.position - position5) * 15f * myLevel,
							true,
							(!nonAI) ? (-1) : base.photonView.viewID,
							base.name,
							true
			...
			else if (attackAnimation == "jumper_1" || attackAnimation == "crawler_jump_1")
			{
				if (baseAnimation["attack_" + attackAnimation].normalizedTime >= 1f && grounded)
				{
					...
					GameObject gameObject7 = ((IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.MULTIPLAYER || !base.photonView.isMine) ? ((GameObject)UnityEngine.Object.Instantiate(Resources.Load("FX/boom2"), fxPosition, fxRotation)) : PhotonNetwork.Instantiate("FX/boom2", fxPosition, fxRotation, 0));
					gameObject7.transform.localScale = baseTransform.localScale * 1.6f;
					float b2 = 1f - Vector3.Distance(currentCamera.transform.position, gameObject7.transform.position) * 0.05f;
					b2 = Mathf.Min(1f, b2);
		...
		else if (state == TitanState.grab)
		{
			...
			{
				GameObject gameObject8 = checkIfHitHand(currentGrabHand);
				if (gameObject8 != null)
				{
					if (isGrabHandLeft)
					{
						eatSetL(gameObject8);
						grabbedTarget = gameObject8;
					}
					else
					{
						eatSet(gameObject8);
		...
		else if (state == TitanState.chase)
		{
			...
			else
			{
				...
				else if (abnormalType == AbnormalType.TYPE_CRAWLER)
				{
					...
					float f = 0f - Mathf.DeltaAngle((0f - Mathf.Atan2(vector5.z, vector5.x)) * 57.29578f, baseGameObjectTransform.rotation.eulerAngles.y - 90f);
					...
					GameObject gameObject9 = checkIfHitCrawlerMouth(head, 2.2f);
					if (gameObject9 != null)
					{
						Vector3 position6 = baseTransform.Find("Amarture/Core/Controller_Body/hip/spine/chest").position;
						if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
						{
							gameObject9.GetComponent<HERO>().die((gameObject9.transform.position - position6) * 15f * myLevel, isBite: false);
						}
						else if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER && base.photonView.isMine)
						{
							...
							else if (!gameObject9.GetComponent<HERO>().HasDied())
							{
								...
								{
									(gameObject9.transform.position - position6) * 15f * myLevel,
									true,
									(!nonAI) ? (-1) : base.photonView.viewID,
									base.name,
									true
		...
		else if (state == TitanState.wander)
		{
			...
			if (myDistance < chaseDistance || whoHasTauntMe != null)
			{
				...
				num15 = 0f - Mathf.DeltaAngle((0f - Mathf.Atan2(vector6.z, vector6.x)) * 57.29578f, baseGameObjectTransform.rotation.eulerAngles.y - 90f);
				if (isAlarm || Mathf.Abs(num15) < 90f)
				{
					chase();
					return;
				}
