...
public static class NGUIMath
{
	...
	public static Bounds CalculateAbsoluteWidgetBounds(Transform trans)
	{
		...
		for (int num = componentsInChildren.Length; i < num; i++)
		{
			...
			Vector2 relativeSize = obj.relativeSize;
			Vector2 pivotOffset = obj.pivotOffset;
			float num2 = (pivotOffset.x + 0.5f) * relativeSize.x;
			float num3 = (pivotOffset.y - 0.5f) * relativeSize.y;
			relativeSize *= 0.5f;
			Transform cachedTransform = obj.cachedTransform;
			Vector3 lhs = cachedTransform.TransformPoint(new Vector3(num2 - relativeSize.x, num3 - relativeSize.y, 0f));
			vector2 = Vector3.Max(lhs, vector2);
			vector = Vector3.Min(lhs, vector);
			Vector3 lhs2 = cachedTransform.TransformPoint(new Vector3(num2 - relativeSize.x, num3 + relativeSize.y, 0f));
			vector2 = Vector3.Max(lhs2, vector2);
			vector = Vector3.Min(lhs2, vector);
			Vector3 lhs3 = cachedTransform.TransformPoint(new Vector3(num2 + relativeSize.x, num3 - relativeSize.y, 0f));
			vector2 = Vector3.Max(lhs3, vector2);
			vector = Vector3.Min(lhs3, vector);
			Vector3 lhs4 = cachedTransform.TransformPoint(new Vector3(num2 + relativeSize.x, num3 + relativeSize.y, 0f));
			vector2 = Vector3.Max(lhs4, vector2);
			vector = Vector3.Min(lhs4, vector);
		}
