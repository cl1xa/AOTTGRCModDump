...
public class Emitter
{
	...
	public void SetEmitPosition(EffectNode node)
	{
		...
		else if (Layer.EmitType == 4)
		{
			...
			Vector3 vector4 = Layer.EmitPoint + Layer.ClientTransform.localRotation * Vector3.forward * Layer.LineLengthRight - vector3;
		...
		else if (Layer.EmitType == 3)
		{
			...
			float y2 = 360f * num3;
			Vector3 vector5 = Quaternion.Euler(0f, y2, 0f) * (Vector3.right * Layer.Radius);
			vector = Quaternion.FromToRotation(Vector3.up, Layer.CircleDir) * vector5;
			if (!Layer.SyncClient)
			{
