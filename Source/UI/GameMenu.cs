...
internal class GameMenu : BaseMenu
{
	...
	{
		EmoteTextPopup obj = (EmoteTextPopup)GetAvailablePopup(_emoteTextPopups);
		if (text.Length > 20)
		{
	...
	{
		((EmoteEmojiPopup)GetAvailablePopup(_emoteEmojiPopups)).Show(emoji, parent);
	}
	...
	private static List<string> GetEmoteWheelOptions(EmoteWheelState state)
	{
		return state switch
		{
			EmoteWheelState.Text => AvailableText, 
			EmoteWheelState.Emoji => AvailableEmojis, 
			_ => AvailableActions, 
		};
	}
	...
	protected void SetupCrosshairs()
	{
		...
		CursorManager.UpdateCrosshair(_crosshairImageWhite, _crosshairImageRed, _crosshairLabelWhite, _crosshairLabelRed, force: true);
	}
