...
public class BTN_save_snapshot : MonoBehaviour
{
	...
	private IEnumerator ScreenshotEncode()
	{
		...
		Texture2D texture2D = new Texture2D((int)(num * targetTexture.transform.localScale.x), (int)(num * targetTexture.transform.localScale.y), TextureFormat.RGB24, mipmap: false);
		...
		info.GetComponent<UILabel>().text = $"Saved snapshot to {SnapshotManager.SnapshotPath}";
	}
}
