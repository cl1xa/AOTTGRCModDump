...
internal class HERO : Photon.MonoBehaviour
{
	...
	private void CreateAndAttachThunderSpears()
	{
		...
		AttachThunderSpear(ThunderSpearL, handL.transform, left: true);
		AttachThunderSpear(ThunderSpearR, handR.transform, left: false);
			...
			if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE || base.photonView.isMine)
			{
				...
				else
				{
					...
					else
					{
						...
						if ((state == HERO_STATE.Idle && !baseAnimation.IsPlaying("dash") && !baseAnimation.IsPlaying("wallrun") && !baseAnimation.IsPlaying("toRoof") && !baseAnimation.IsPlaying("horse_geton") && !baseAnimation.IsPlaying("horse_getoff") && !baseAnimation.IsPlaying("air_release") && !isMounted && (!baseAnimation.IsPlaying("air_hook_l_just") || baseAnimation["air_hook_l_just"].normalizedTime >= 1f) && (!baseAnimation.IsPlaying("air_hook_r_just") || baseAnimation["air_hook_r_just"].normalizedTime >= 1f)) || baseAnimation["dash"].normalizedTime >= 0.99f)
						{
							...
							else
							{
								...
								{
									float num9 = 0f - Mathf.DeltaAngle((0f - Mathf.Atan2(baseRigidBody.velocity.z, baseRigidBody.velocity.x)) * 57.29578f, baseTransform.rotation.eulerAngles.y - 90f);
						...
						else if (!baseAnimation.IsPlaying("attack5") && !baseAnimation.IsPlaying("special_petra") && !baseAnimation.IsPlaying("dash") && !baseAnimation.IsPlaying("jump") && !IsFiringThunderSpear())
						{
							...
							Vector3 globaleFacingVector = getGlobaleFacingVector3(num10);
					...
					if (flag2 && flag3)
					{
						...
						baseRigidBody.AddForce(-baseRigidBody.velocity, ForceMode.VelocityChange);
						Vector3 current = (bulletRight.transform.position + bulletLeft.transform.position) * 0.5f - baseTransform.position;
						float reelAxis = GetReelAxis();
						reelAxis = Mathf.Clamp(reelAxis, -0.8f, 0.8f);
						float num13 = 1f + reelAxis;
						Vector3 vector8 = Vector3.RotateTowards(current, baseRigidBody.velocity, 1.53938f * num13, 1.53938f * num13);
						vector8.Normalize();
						spinning = true;
					...
					else if (flag2)
					{
						...
						baseRigidBody.AddForce(-baseRigidBody.velocity, ForceMode.VelocityChange);
						Vector3 current2 = bulletLeft.transform.position - baseTransform.position;
						float reelAxis2 = GetReelAxis();
						reelAxis2 = Mathf.Clamp(reelAxis2, -0.8f, 0.8f);
						float num15 = 1f + reelAxis2;
						Vector3 vector9 = Vector3.RotateTowards(current2, baseRigidBody.velocity, 1.53938f * num15, 1.53938f * num15);
						vector9.Normalize();
						spinning = true;
					...
					else if (flag3)
					{
						...
						baseRigidBody.AddForce(-baseRigidBody.velocity, ForceMode.VelocityChange);
						Vector3 current3 = bulletRight.transform.position - baseTransform.position;
						float reelAxis3 = GetReelAxis();
						reelAxis3 = Mathf.Clamp(reelAxis3, -0.8f, 0.8f);
						float num17 = 1f + reelAxis3;
						Vector3 vector10 = Vector3.RotateTowards(current3, baseRigidBody.velocity, 1.53938f * num17, 1.53938f * num17);
						vector10.Normalize();
						spinning = true;
	...
	private void headMovement()
	{
		...
		float value2 = Mathf.Atan2(obj.position.y - gunTarget.y, x) * 57.29578f;
	...
	public void lateUpdate2()
	{
		...
		if (leftArmAim || rightArmAim)
		{
			...
			float num2 = 0f - Mathf.DeltaAngle((0f - Mathf.Atan2(vector9.z, vector9.x)) * 57.29578f, baseTransform.rotation.eulerAngles.y - 90f);
			headMovement();
	...
	private void Start()
	{
		...
		{
			SetInterpolationIfEnabled(interpolate: true);
	...
	private void LaunchThunderSpear()
	{
		...
		Vector3 position;
		if (Vector3.Cross(baseTransform.forward, vector2).y < 0f && state != HERO_STATE.Land)
		{
			position = ThunderSpearL.transform.position;
			ThunderSpearL.audio.Play();
			SetThunderSpears(hasLeft: false, hasRight: true);
			attackAnimation = "AHSS_shoot_l";
		}
		else
		{
			...
			SetThunderSpears(hasLeft: true, hasRight: false);
			attackAnimation = "AHSS_shoot_r";
		}
	...
	public void UpdateThunderSpear()
	{
		...
		{
			SetThunderSpears(hasLeft: true, hasRight: true);
		}
