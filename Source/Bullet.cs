...
internal class Bullet : Photon.MonoBehaviour
{
	...
	public void checkTitan()
	{
		...
		if (!Physics.Raycast(base.transform.position, velocity, out var hitInfo, 10f, layerMask.value))
		{
			return;
		}
	...
	public void update()
	{
		...
		else
		{
			...
			else if (phase == 1)
			{
				...
				Vector3 vector2 = master.rigidbody.velocity;
				...
				for (; i < value; i++)
				{
					...
					lineRenderer.SetPosition(i, new Vector3(Random.Range(0f - num4, num4), Random.Range(0f - num4, num4), Random.Range(0f - num4, num4)) + myRef.transform.position + vector * ((float)i / (float)value) - Vector3.up * num * 0.05f * f - vector2 * 0.001f * f * num);
				}
			...
			else if (phase == 2)
			{
				...
				{
					getSpiral(master.transform.position, master.transform.rotation.eulerAngles);
					Vector3 vector3 = myRef.transform.position - (Vector3)spiralNodes[0];
					lineRenderer.SetVertexCount(spiralNodes.Count - (int)((float)spiralcount * 0.5f));
					for (int j = 0; (float)j <= (float)(spiralNodes.Count - 1) - (float)spiralcount * 0.5f; j++)
					{
						if (spiralcount < 5)
						{
							Vector3 vector4 = (Vector3)spiralNodes[j] + vector3;
							...
							lineRenderer.SetPosition(j, vector4);
						}
						else
						{
							lineRenderer.SetPosition(j, (Vector3)spiralNodes[j] + vector3);
						}
					}
				}
			}
